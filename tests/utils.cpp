...
TEST(UtilTest, SplitStringSimple) {
    ...
    auto [properly_quoted2, parts2] = split_string_into_parts("", " ");
    EXPECT_TRUE(properly_quoted2);
    EXPECT_EQ(parts2, std::vector<std::string>());
    auto [properly_quoted3, parts3] = split_string_into_parts("hello \"world\"", " ");
    EXPECT_TRUE(properly_quoted3);
    EXPECT_EQ(parts3.size(), 2);
    EXPECT_EQ(parts3, std::vector<std::string>({"hello", "world"}));
    auto [properly_quoted4, parts4] = split_string_into_parts("hello \"world", " ");
    EXPECT_FALSE(properly_quoted4);
    auto [properly_quoted5, parts5] = split_string_into_parts("\"\"", " ");
...
TEST(SplitNasty, MultipleConsecutiveSpaces) {
    auto [ok, parts] = split_string_into_parts("hello    world", " ");
    EXPECT_TRUE(ok);
    EXPECT_EQ(parts, std::vector<std::string>({"hello", "world"}));
}
...
TEST(SplitNasty, OnlySpaces) {
    auto [ok, parts] = split_string_into_parts("     ", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, QuotedStringPreservesSpaces) {
    auto [ok, parts] = split_string_into_parts("echo \"hello   world\"", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, QuotedStringPreservesEdgeSpaces) {
    auto [ok, parts] = split_string_into_parts("echo \"  padded  \"", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, SandwichedQuote) {
    auto [ok, parts] = split_string_into_parts("a\"b c\"d", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, EmptyQuotedAmongOthers) {
    auto [ok, parts] = split_string_into_parts("a \"\" b", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, SingleQuoteIsLiteral) {
    auto [ok, parts] = split_string_into_parts("it's alive", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, CommaDelimiter) {
    auto [ok, parts] = split_string_into_parts("a,b,c", ",");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, SingleChar) {
    auto [ok, parts] = split_string_into_parts("a", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, NewlineInInput) {
    auto [ok, parts] = split_string_into_parts("hello\nworld", " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, EmbeddedNullByte) {
    ...
    auto [ok, parts] = split_string_into_parts(evil, " ");
...
TEST(SplitNasty, BackslashBeforeQuote) {
    auto [ok, parts] = split_string_into_parts("echo \\\"hello\\\"", " ");
...
TEST(SplitNasty, VeryLongInput) {
    ...
    auto [ok, parts] = split_string_into_parts(huge, " ");
    EXPECT_TRUE(ok);
    EXPECT_EQ(parts.size(), 2);
...
TEST(SplitNasty, ManySmallTokens) {
    ...
    auto [ok, parts] = split_string_into_parts(input, " ");
    EXPECT_TRUE(ok);
...
TEST(SplitNasty, QuoteInMiddleOfWord) {
    auto [ok, parts] = split_string_into_parts("he\"ll\"o", " ");
    EXPECT_TRUE(ok);
    EXPECT_EQ(parts, std::vector<std::string>({"hello"}));
}
